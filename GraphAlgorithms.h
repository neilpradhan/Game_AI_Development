...
private:
  ...
  //to aid legibility
  enum {visited, unvisited, no_parent_assigned};// 0,1,2;
...
private:
  ...
  // here I will create flag function for Iterative Deepening Depth First traversal
  // 1st int is the source index 2nd is target index and third is limit 
...
// constructor
  ...
  std::vector<const Edge*> GetSearchTree()const{return m_SpanningTree;}// most likely for visual purposes

  //returns true if the target node has been located
  bool   Found()const{return m_bFound;}// depends on search()

  //returns a vector of node indexes that comprise the shortest path
  //from the source to the target
  std::list<int> GetPathToTarget()const; // final path required 
};
...
template <class graph_type>
bool Graph_SearchDFS<graph_type>:: Depth_Limited_Search(int src, int target, int limit)
{
	if (src == target)
		return true;
	...
	// Recur for all the vertices adjacent to source edges
	graph_type::ConstEdgeIterator ConstEdgeItr(m_Graph,src);
	for (const Edge* pE = ConstEdgeItr.begin();
		!ConstEdgeItr.end();
		pE = ConstEdgeItr.next())
		if (Depth_Limited_Search(pE->To(), target, limit - 1) == true)
			return true;
 ...
 while (!stack.empty())
  {
	...
	//Graph_SearchDFS<graph_type>:: Depth_Limited_Search ( m_iSource,  m_iTarget, 10);
